// IoT07-2 ESP32 WebServer - SW Access Point
#define SWAP 1  // SWAP 모드 설정 (1: AP 모드, 0: 클라이언트 모드)

// Wi-Fi 라이브러리 로드
#include <WiFi.h>

// Wi-Fi 네트워크 정보 설정 (SWAP 모드에서는 AP로 동작)
#if SWAP
const char* ssid = "ESP32-AP";  // AP 모드에서 생성할 네트워크 SSID
...
void setup() {
  ...
  IPAddress IP = WiFi.softAPIP();  // AP 모드의 IP 주소 가져오기
